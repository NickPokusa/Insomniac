...
if (fading_out) {
	...
	if(image_alpha <= 0){
		...
		var light = instance_find(FlashlightLightNight3obj, 0);
		if (light != noone) {
			light.visible = false;
		}
		
		//Reset flashlight toggle state
		var flash = instance_find(FlashlightNight3Obj, 0);
		if (flash != noone){
		...
		alarm[0] = irandom_range(game_get_speed(gamespeed_fps) * 6, game_get_speed(gamespeed_fps) * 10);
	}
